...
**NOTE** : The only `ee.Classifiers`(s) that can be exported are `ee.Classifier.smileRandomForest`, `ee.Classifier.smileCart`, `ee.Classifier.DecisionTree` and `ee.Classifier.DecisionTreeEnsemble`. 
### Code Editor (JavaScript)
```
